{
    ...
    const getMousePos = (ev) => {
        ...
        if (ev.pageX || ev.pageY) {
            ...
            posy = ev.clientY + body.scrollTop + docElscrollTop;
        }
    ...
    class CursorFx {
        constructor(el) {
            this.DOM = {el:el};
            this.DOM.toggle = this.DOM.el.querySelector('.cursor_inner--circle');

            this.DOM.title = this.DOM.el.querySelector('.cursor_inner--text');
            this.bounds = {
        ...
        render() {
            ...
            // The position of the title/toggle and the viewport side will determine the speed for both of the elements
            const lerpFactor = {
            ...
            this.lastMousePos.toggle.x = MathUtilis.lerp(this.lastMousePos.toggle.x, mousePos.x - this.bounds.toggle.width/2, lerpFactor.toggle);
            this.lastMousePos.toggle.y = MathUtilis.lerp(this.lastMousePos.toggle.y, mousePos.y - this.bounds.toggle.height/2, lerpFactor.toggle);
            this.lastMousePos.title.x = MathUtilis.lerp(this.lastMousePos.title.x, mousePos.x - this.bounds.title.width/2, lerpFactor.title);
            this.lastMousePos.title.y = MathUtilis.lerp(this.lastMousePos.title.y, mousePos.y - this.bounds.title.height/2, lerpFactor.title);
        ...
        toggle() {
            const isCircle = this.DOM.toggle.classList.contains('cursor_inner--circle');
            this.DOM.toggle.classList[isCircle ? 'remove' : 'add']('cursor_inner--circle');
            this.DOM.toggle.classList[isCircle ? 'add' : 'remove']('cursor_inner--cross');
    ...
    class Grid {
        constructor(el) {
            ...
            this.DOM.contentTitle = this.DOM.content.querySelector('.content_title');
        ...
        calculateSize() {
            ...
            // The anime.js stagger function needs an array [cols,rows]
            this.gridDef = [this.columns, this.itemsTotal/this.columns];
        }
        initEvents() {
            //Window resize event
            // The lerpFactor will change to 1 so theres no delay when translating the grid (or we could see the gaps on the top and bottom)
        ...
        render() {
            ...
            this.gridTranslation.y = MathUtilis.lerp(this.gridTranslation.y, Math.min(Math.max(MathUtilis.lineEq(-1*this.extraHeight, 0, this.height-this.height*.1, this.height*.1, mousePos.y), -1*this.extraHeight),0), this.lerpFactor);
    ...
    imagesLoaded(document.querySelectorAll('.grid_item-inner, img'), {background: true}, () => document.body.classList.remove('loading'));
